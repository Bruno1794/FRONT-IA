'use server'
import {PUT_CLIENTES} from "@/functions/Api";
import {cookies} from "next/headers";
import {parseCurrencyToFloat} from "@/functions/format-money-bd";

export default async function putClient(state: object, formData: FormData) {
    const {url} = PUT_CLIENTES(Number(formData.get('id')));
...
//console.log(formData);
    ...
    try {
        const client = {
            ...
            msg_enviar: formData.get('msg_enviar') as string,
