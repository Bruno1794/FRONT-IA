...
import showClient from "@/action/client/show-client";
import putClient from "@/action/client/put-client";
...
export default function NovoClientePage() {
    ...
    const [avisar, setAvisar] = React.useState<string>("");
    const [referencia, setReferencia] = React.useState<string>("");
    const [msgCobranca, setMsgCobranca] = React.useState<string>("");
    const [observacao, setObservacao] = React.useState<string>("");
    const [cobranca, setCobranca] = React.useState<string>("MENSAL");
    const [preferencia, setPreferencia] = React.useState<string>("");
    const [cobrar, setCobrar] = React.useState<boolean>(false);
    ...
    const [state, actiion] = React.useActionState(putClient, {
        ok: false,
        error: '',
        data: null
    })
    ...
    React.useEffect(() => {
        async function show() {
            const {client} = await showClient(Number(id))
            setNome(client.name ?? '')
            setCpf(formatCPF(client.cpf ?? ''))
            setPhone(formatPhone(client?.phone ?? ''))
            setEmail(client.email ?? '')
            setVencimento(client.vencimento ?? '')
            setValor(formatMoneyInput(client.value_mensalidade ?? ''))
            setAvisar(client.avisar ?? '')
            setReferencia(client.referencia ?? '')
            setCobranca(client.type_cobranca ?? '')
            setMsgCobranca(client.msg_enviar ?? '')
            setObservacao(client.observation ?? '')
            setCobrar(client.cobrar ?? '')
            setPreferencia(client.preferencia ?? '')
        }
        ...
        <>
            ...
            <section className={`conteiner ${styles.section}`}>
                <form className={styles.form} action={actiion}>
                    ...
                    <div className={styles.agrupar}>
                        <div className={styles.formGroup}>
                        <textarea name="msg_enviar" cols={20} rows={5} placeholder=" "
                                  value={msgCobranca}
                                  onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => {
                                      setMsgCobranca(e.target.value)
                                  }}
                                  required={false}></textarea>
                            <label>Mensagem de Cobrança</label>
                        </div>
                        <div className={styles.formGroup}>
                        <textarea name="observation" id="" cols={20} rows={5} placeholder=" "
                                  value={observacao}
                                  onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => {
